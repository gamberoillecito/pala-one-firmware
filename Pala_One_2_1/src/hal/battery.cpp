...
static uint32_t batteryCacheIntervalMs() {
  // Battery-only mode can stay stale for minutes, but once the voltage is up
  // in the charger range we want the icon to track that state much sooner.
  if (s_battery.usb || s_battery.filteredV >= 4.05f) return 5000;
  return BAT_CACHE_MS;
}

void updateBatteryCached(bool force) {
  uint32_t now = millis();
  if (!force && (now - s_battery.lastMs) < batteryCacheIntervalMs()) return;
  s_battery.lastMs = now;

  float raw = readBatteryVoltageRaw();
  s_battery.rawV = raw;
  s_battery.usb = (raw >= 4.25f);
...
void drawBatteryTopRight() {
  ...
  bool usbActive = s_battery.usb;

  int pct = s_battery.valid ? s_battery.pctShown : 0;
  if (usbActive) pct = 100;
  if (pct < 0) pct = 0;
  if (pct > 100) pct = 100;
  ...
  Font::useUiSmall();
  char buf[8];
  if (usbActive) snprintf(buf, sizeof(buf), "USB");
  else if (s_battery.valid) snprintf(buf, sizeof(buf), "%d%%", pct);
